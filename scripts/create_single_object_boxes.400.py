...
pink = [(226,204,225),(200,99.976),(131,241), (368.97, 241)]
black = [(0,0,0), (298.345,101.655)]
green = [(186, 216,115), (300, 200), (259,131), (259,369)]
white = [(255,255,255), (298.345,298.345)]
orange = [(244,185,126), (200,300), (131, 259), (368.97, 259)]
sky = [(152, 217, 238), (101.655, 298.345)]
purple = [(180, 43, 144), (99.977, 200), (241, 131), (241, 369)]
yellow = [(244, 235, 47), (101.655,101.655)]
boxes = dict(pink=pink, black=black, green=green, white=white, orange=orange, sky=sky, purple=purple, yellow=yellow)

##round centers to integer locations
for bx in boxes.keys():
  boxes[bx][1] = tuple(map(round, boxes[bx][1]))

##a helper in case you want to show more of the boxes...probably doesn't work right now...  
...
imagery_run_folders = glob.glob('/Users/tnaselar/Data/Presentation/single.object.boxes/pics/single.object.400/imagery_*')
...
save_to_dir = '/Users/tnaselar/Data/Presentation/single.object.boxes/pics/single.object.400/'
...
background_image_file = '/Users/tnaselar/Data/Presentation/single.object.boxes/Boxes.400.png'
rest_label = 'XXXXXX'
...
#   list_of_images = [list_of_images[0]]
  ...
  for offsets in jitters:
    ...
    imo.put_text(copy_background, message=rest_label, position=(0.5,0.5)).save(save_to+'/frame_files/'+'rest_frame_%d.png' % (cnt))
    cnt += 1
